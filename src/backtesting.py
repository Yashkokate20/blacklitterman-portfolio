...
class BacktestEngine:
    ...
    def _get_rebalance_dates(self) -> List[pd.Timestamp]:
        ...
        if self.rebalance_frequency == "M":
            ...
            return (
                pd.date_range(
                    start=self.returns.index[0], end=self.returns.index[-1], freq="M"
                )
            ...
            return (
                pd.date_range(
                    start=self.returns.index[0], end=self.returns.index[-1], freq="Q"
                )
                .intersection(self.returns.index)
                .tolist()
            )
    ...
    def backtest_strategy(
        self, strategy_weights: pd.Series, strategy_name: str = "Strategy"
    ) -> Dict:
        """
        ...
        metrics = self._calculate_performance_metrics(
            portfolio_returns, cumulative_returns, strategy_name
        )
    ...
    def backtest_dynamic_strategy(
        self, weight_function, strategy_name: str = "Dynamic Strategy", **kwargs
    ) -> Dict:
        """
        ...
        **kwargs : dict
        ...
        if turnover_history:
            ...
            metrics["total_transaction_costs"] = sum(
                t * self.transaction_cost for t in turnover_history
            )
    ...
    def _calculate_performance_metrics(
        self, returns: pd.Series, cumulative_returns: pd.Series, strategy_name: str
    ) -> Dict:
    ...
    def compare_strategies(self, strategies_results: List[Dict]) -> pd.DataFrame:
        ...
        for result in strategies_results:
            ...
            if "avg_turnover" in metrics:
                comparison_data[strategy_name]["Avg Turnover (%)"] = (
    ...
    def plot_performance(self, strategies_results: List[Dict], figsize=(15, 10)):
        ...
        for result in strategies_results:
            ...
            rolling_sharpe = (returns.rolling(252).mean() * 252) / (
