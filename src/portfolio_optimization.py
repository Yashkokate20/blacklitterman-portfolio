...
class PortfolioOptimizer:
    ...
    def optimize_unconstrained(self, risk_aversion: float = 3.0) -> pd.Series:
        ...
        try:
            ...
            optimal_weights = (1 / risk_aversion) * np.dot(inv_cov, self.expected_returns.values)
            ...
            optimal_weights = (1 / risk_aversion) * np.dot(pinv_cov, self.expected_returns.values)
            return pd.Series(optimal_weights, index=self.assets)
    
    def optimize_constrained(self, 
                           constraints: Dict = None,
                           risk_aversion: float = 3.0,
                           target_return: Optional[float] = None) -> Tuple[pd.Series, Dict]:
        """
        ...
        try:
            ...
            port_risk = np.sqrt(np.dot(optimal_weights, np.dot(self.covariance_matrix, optimal_weights)))
    ...
    def efficient_frontier(self, 
                          n_points: int = 50,
                          constraints: Dict = None) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def compute_portfolio_stats(self, weights: pd.Series) -> Dict:
        ...
        portfolio_variance = np.dot(aligned_weights, np.dot(self.covariance_matrix, aligned_weights))
