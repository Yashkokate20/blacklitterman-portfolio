...
def validate_matrix_properties(matrix: np.ndarray, 
                             ...
                             condition_threshold: float = 1e12) -> Dict[str, bool]:
    """
    ...
    if check_positive_definite and results['is_square'] and results['is_symmetric']:
        try:
            ...
            if not results['is_positive_definite']:
                logger.warning(f"{name} is not positive definite (min eigenvalue: {results['min_eigenvalue']:.2e})")
        except:
    ...
    if results['is_square']:
        try:
            ...
            results['is_well_conditioned'] = results['condition_number'] < condition_threshold
            
            if not results['is_well_conditioned']:
                logger.warning(f"{name} is poorly conditioned (condition number: {results['condition_number']:.2e})")
        except:
...
def clean_returns_data(returns: pd.DataFrame,
    ...
    """
    logger.info(f"Cleaning returns data: {returns.shape[0]} obs × {returns.shape[1]} assets")
    ...
    logger.info(f"Cleaned data: {cleaned_returns.shape[0]} obs × {cleaned_returns.shape[1]} assets")
    
    return cleaned_returns

def estimate_robust_covariance(returns: pd.DataFrame, 
    ...
    if method == 'ledoit_wolf':
        ...
        robust_cov_matrix, shrinkage = lw_estimator.fit(returns.values).covariance_, lw_estimator.shrinkage_
...
def generate_synthetic_data(tickers: List[str],
                          ...
                          end_date: str = None,
                          n_regimes: int = 3) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series]:
    """
    ...
    for regime in range(n_regimes):
        ...
        if regime == 0:  # Bull market
            ...
            mean_returns = np.random.uniform(-0.0010, 0.0005, n_assets)  # Negative returns
            ...
            mean_returns = np.random.uniform(0.0005, 0.0010, n_assets)  # Moderate returns
        ...
        logger.info(f"Regime {regime + 1}: {regime_days} days, "
    ...
    returns = pd.DataFrame(all_returns, index=dates[:len(all_returns)], columns=tickers)
...
def load_market_data(tickers: List[str],
                    ...
                    end_date: str = None,
                    fallback_market_caps: Dict[str, float] = None) -> Tuple[pd.DataFrame, pd.DataFrame, pd.Series]:
    """
...
def calculate_performance_metrics(returns: pd.Series,
    ...
    if len(downside_returns) > 0:
        ...
        metrics['sortino_ratio'] = (annual_return - risk_free_rate) / downside_vol if downside_vol > 0 else 0
    ...
    metrics.update({
        ...
        'calmar_ratio': abs(annual_return / metrics['max_drawdown']) if metrics['max_drawdown'] != 0 else 0,
    })
